...
def display_image():
    ...
    status.configure(text= f'In: {inH}x{inW}, Out: {outH}x{outW}, file path:{filename}')
    VIEW_X, VIEW_Y = 512, 512
...
if __name__ == '__main__':
    ...
    window.title('Computer Vision (Ver 0.08)')
    ...
    comVisionMenu5.add_separator()
    comVisionMenu5.add_command(label='Save as Excel file', command=save_as_excel)
    comVisionMenu5.add_command(label='Save as Excel Art', command=save_as_excel_art)
    comVisionMenu5.add_command(label='Open Excel file', command=open_excel)
